<template>
  <div>
    <div :class="type.class" @click="onClick">
      <div class="row">
        ...
        <div :class="'col row justify-end content-center ' + type.color.mainClass">
          <ConsumerGroupStatus v-if="deployedTopology.aggregatedConsumerGroupStatus" :status="deployedTopology.aggregatedConsumerGroupStatus" />
        </div>
        <div :class="'row flex-center deployment-metadata ' + type.color.mainClass">
          <div>
    ...
    <DeployedTopologyDeploymentActionItemTooltip v-if="hasActionResults" :actionResults="actionResults" />
  </div>
</template>
...
export default {
  ...
  components: { DeployedTopologyDeploymentActionItemTooltip, ConsumerGroupStatus },
  props: {
  ...
  methods: {
    onClick(event) {
      this.$router.push({ path: `/deployed/${this.deployedTopology.kafkaClusterId}/${this.deployedTopology.topologyId}` });
    }
  },
  computed: {
    successType() {
    ...
    type() {
      if (this.deployedTopology.failedOperations.length > 0) {
        return this.errorType;
      } else if (this.deployedTopology.unsafeOperations.length > 0) {
...
.deployment:hover {
  cursor: pointer;
  background: lighten($card-deployment-bg, 10%);
}
.deployment:hover .bg-deployment-error-side { background: lighten($card-error-side-bg, 10%); }
.deployment:hover .bg-deployment-error-main { background: lighten($card-error-main-bg, 10%); }
.deployment:hover .consumergroupstatus-info { background: darken($card-success-icon, 40%); }
.deployment:hover .consumergroupstatus-warning { background: darken($card-warning-icon, 50%); }
.deployment:hover .consumergroupstatus-error { background: lighten($card-error-side-bg, 10%); }
